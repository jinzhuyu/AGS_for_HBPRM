...
cal_exact_var = function(x){
  if(x<1){
    stop('Error: the input must be >=1 in calculating the exact mean of the approximate distribution')
  }
  if (x>=2){
    seq_neg_part = seq(1,x-1)
...
# usage: prop_approx_likeli(coeff_curr, coeff_curr_id=i_coeff, hyper_param_curr=hyper_param_curr[i_coeff,])
# the prop_normal does not depend on other coeff and hyper params
## note: To improve the approximation accuracy - replace log(y) with exact_mean
## and replace y with 1/exact_var
...
# usage: 
# gibbs_without_mh(
#   coeff_curr = coeff[, i_samp-1, , i_chain, i_meth], hyper_param_curr = hyper_param[ ,i_samp-1, ,i_chain, i_meth])
  ...
  for(i_meth in 1:n_meth){
    ...
    for (i_chain in 1:n_chain){
      ...
      for (i_samp in 2:n_total_sample){
        ...
        # if (sum(is.na(hyper_param[, i_samp, 2, i_chain, i_meth]))>=1){
        #   coeff_0 = coeff[, i_samp-1, , i_chain, i_meth]
        #   hyper_param_0 = hyper_param[ ,i_samp-1, ,i_chain, i_meth]
        #   print(i_samp)
        #   break
        # }
