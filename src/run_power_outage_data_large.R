...
# import data
data_list = read.csv("C:/GitHub/approx_gibbs_for_HBM/test_data/power_outage_data_large/outage_corr_atmos_ext_df.csv", header = T)

# for (i in 1:max(data_list$event_id)){
...
# try to change the group-level values to see the impact
# for (i in 1:max(group_id)){
#   data_list_select$max_outage_count[group_id==i] = abs(rnorm(1,0,0.5))
# }

# # scale group covariate
data_list_select[, n_cov] = min_max(data_list_select[, n_cov])

# let count==3 if if count<3
ub_approx = 3
data_list_select$outage_count[data_list_select$outage_count<ub_approx] = ub_approx

# data_list_select = data_list_select[group_id %in% c(14),]

# # scale up y does not work
# data_list_select$outage_count = data_list_select$outage_count*100
